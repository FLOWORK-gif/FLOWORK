...
class LogicToolboxWidget(BaseDashboardWidget):
    ...
    (FIXED) Refresh logic is now separated for manual trigger vs. event-driven update to prevent loops.
    """
    ...
    def _load_data_worker(self, force_reload: bool = False):
        if force_reload:
            # 1. Trigger a hot reload across the application
            self.api_client.trigger_hot_reload()
            # 2. Give the system a moment to rescan everything
            time.sleep(1)

        # 3. Fetch the (potentially) updated list
    ...
    # (MODIFIED) This function, called by the event bus, now only refreshes data without forcing a reload.
    def refresh_content(self, event_data=None):
        self.kernel.write_to_log("LogicToolboxWidget received signal to refresh from EventBus.", "INFO")
