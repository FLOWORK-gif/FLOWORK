...
# File NAME : C:\FLOWORK\widgets\logic_toolbox_widget\processor.py
# JUMLAH BARIS : 124
#######################################################################
...
class LogicToolboxWidget(BaseDashboardWidget):
    ...
    (MODIFIED) Now strictly displays ONLY components from the 'modules' directory.
    """
    ...
    def populate_module_toolbox(self, success=True, all_modules_data=None):
        ...
        if not success:
            ...
            return
        logic_modules_data = all_modules_data
        modules_to_display = []
        if not search_query:
