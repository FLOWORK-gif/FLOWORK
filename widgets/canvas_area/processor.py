...
# File NAME : C:\FLOWORK\widgets\canvas_area\processor.py
# JUMLAH BARIS : 349
#######################################################################
...
import json
from widgets.data_canvas_widget.processor import DataCanvasWidget # [FIXED] Corrected the import path
class CanvasAreaWidget(BaseDashboardWidget):
    ...
    def _create_widgets(self):
        ...
        ttk.Label(preset_action_frame, text=self.loc.get('load_preset_label', fallback="Load Preset:")).pack(side='left') # English Fallback
        ...
        self.delete_preset_button = ttk.Button(preset_action_frame, text=self.loc.get('delete_preset_button', fallback="Delete Preset"), command=self._delete_selected_preset, style='danger.TButton') # English Fallback
        self.delete_preset_button.pack(side='left', padx=(0, 5))
        self.save_preset_button = ttk.Button(preset_action_frame, text=self.loc.get('save_preset_button', fallback="Save as Preset"), command=self._save_as_preset, style='primary.TButton') # English Fallback
        ...
        management_group = ttk.LabelFrame(self.floating_controls_panel, text=self.loc.get('workflow_management_title', fallback="Workflow Management"), bootstyle="dark") # English Fallback
        management_group.pack(side='left', padx=(0, 10), fill='y')
        save_button = ttk.Button(management_group, text=self.loc.get('save_workflow_button', fallback="💾 Save"), command=self.parent_tab.action_handler.save_workflow, style='success.TButton') # English Fallback
        save_button.pack(side='left', padx=5, pady=5)
        load_button = ttk.Button(management_group, text=self.loc.get('load_workflow_button', fallback="📂 Load"), command=self.parent_tab.action_handler.load_workflow, style='info.TButton') # English Fallback
        load_button.pack(side='left', padx=5, pady=5)
        clear_button = ttk.Button(management_group, text=self.loc.get('clear_canvas_button', fallback="🧹 Clear"), command=self.parent_tab.action_handler.clear_canvas, style='danger.TButton') # English Fallback
        clear_button.pack(side='left', padx=5, pady=5)
        execution_control_group = ttk.LabelFrame(self.floating_controls_panel, text=self.loc.get('execution_control_title', fallback="Execution Control"), bootstyle="dark") # English Fallback
        ...
        ttk.Label(loop_count_frame, text=self.loc.get('loop_run_label', fallback="Repeat:"), style='inverse-dark.TLabel').pack(side='left', padx=(0, 5)) # English Fallback
        loop_entry = ttk.Entry(loop_count_frame, textvariable=self.loop_count_var, width=5)
        loop_entry.pack(side='left')
        ttk.Label(loop_count_frame, text=self.loc.get('loop_times_label', fallback="times"), style='inverse-dark.TLabel').pack(side='left', padx=(5, 10)) # English Fallback
        ...
        ttk.Checkbutton(delay_options_frame, text=self.loc.get('enable_delay_checkbox', fallback="Delay Between Loops"), variable=self.enable_delay_var, style='dark.TCheckbutton').pack(side='left') # English Fallback
        ...
        delay_type_radio_static = ttk.Radiobutton(delay_options_frame, text=self.loc.get('delay_type_static', fallback="Static"), variable=self.delay_type_var, value='static', style='dark.TRadiobutton') # English Fallback
        ...
        delay_type_radio_random = ttk.Radiobutton(delay_options_frame, text=self.loc.get('delay_type_random', fallback="Random"), variable=self.delay_type_var, value='random', style='dark.TRadiobutton') # English Fallback
        ...
        self.simulate_button = ttk.Button(execution_buttons_frame, text=self.loc.get('simulate_workflow_button', fallback="Run Simulation"), command=self.parent_tab.action_handler.simulate_workflow, style='primary.TButton') # English Fallback
        self.simulate_button.pack(side='left', expand=True, fill='x', padx=(0,5))
        self.run_button = ttk.Button(execution_buttons_frame, text=self.loc.get('btn_run_workflow', fallback="Run Workflow"), command=self.parent_tab.action_handler.run_workflow, style='success.TButton') # English Fallback
        self.run_button.pack(side='left', expand=True, fill='x', padx=(0,5))
        self.pause_resume_button = ttk.Button(execution_buttons_frame, text=self.loc.get('btn_pause', fallback="Pause"), command=self.parent_tab.action_handler.pause_workflow, style='info.TButton', state='disabled') # English Fallback
    ...
    def _create_debugger_widgets(self):
        ...
        close_button = ttk.Button(controls_frame, text=self.loc.get('debugger_close_button', fallback="Close Debugger"), command=self.hide_debugger, bootstyle="secondary-outline")
        close_button.pack(pady=5)
        payload_frame = ttk.LabelFrame(self.debugger_frame, text=self.loc.get('debugger_payload_title', fallback="Payload at Selected Step"))
