...
# (MODIFIED: The custom importer now recognizes all compiled extensions)
#######################################################################
...
def check_and_install_dependencies():
    ...
    for lib_import_name, lib_pip_name in required_libs.items():
        ...
        if spec is None:
            all_good = False
            log_message(f"[WARN] Required library '{lib_import_name}' not found. Installing...") # English Log
            try:
                command = [sys.executable, "-m", "pip", "install", lib_pip_name, "--quiet", "--disable-pip-version-check"]
                # [FIX] Added creationflags to prevent console window from appearing on Windows
                creation_flags = 0
                if sys.platform == "win32":
                    creation_flags = subprocess.CREATE_NO_WINDOW

                subprocess.check_call(command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=creation_flags) # ADDED creationflags
                log_message(f"[SUCCESS] Library '{lib_pip_name}' installed successfully.") # English Log
            except subprocess.CalledProcessError:
                log_message(f"[FATAL] Failed to install '{lib_pip_name}'. Please check internet connection.") # English Log
                sys.exit(1)
        else:
            log_message(f"[INFO] Library '{lib_import_name}' is already installed.") # English Log

    if all_good:
        log_message("[SUCCESS] All required dependencies are present.") # English Log

    log_message("--- Dependency Check Finished ---") # English Log
...
# [MODIFIKASI] Import dipindahkan ke bawah setelah custom importer aktif
...
from importlib.machinery import SourcelessFileLoader, ExtensionFileLoader # [MODIFIKASI] Tambahkan ExtensionFileLoader
...
def signal_preloader_app_is_ready():
    ...
    try:
        ...
        with open(signal_file_path, "w") as f:
            pass
        print("[INFO] Signal sent to preloader: Application is ready.") # English Log
    except Exception as e:
        print(f"[ERROR] Could not create preloader signal file: {e}") # English Log

# [MODIFIKASI] Importer sekarang jauh lebih pintar
class FloworkFinder(MetaPathFinder):
    """
    A custom import hook that knows how to find and load Flowork's
    proprietary compiled file extensions (.service, .kernel, .aola, etc.).
    """
    def __init__(self, root_path):
        self.root_path = root_path
        # A list of custom extensions to check, in order of preference.
        self.EXTENSIONS_TO_FIND = ['.service', '.kernel', '.aola', '.ai', '.flow', '.teetah']

    def find_spec(self, fullname, path, target=None):
        # We only care about imports within our project's namespace
        if not fullname.startswith('flowork_kernel') and not fullname.startswith('plugins') and not fullname.startswith('widgets') and not fullname.startswith('ai_providers'):
            return None

        module_path_part = fullname.replace('.', os.sep)

        # Loop through our custom extensions
        for ext in self.EXTENSIONS_TO_FIND:
            potential_path = os.path.join(self.root_path, f"{module_path_part}{ext}")
            if os.path.exists(potential_path):
                # We found a compiled file! Load it as a native extension.
                loader = ExtensionFileLoader(fullname, potential_path)
                return importlib.util.spec_from_loader(fullname, loader)

        # If no custom extension is found, return None to let Python's
        # default finders look for a regular .py file.
        return None
...
def run_gui_mode(kernel_instance):
    ...
    kernel_instance.write_to_log("Starting application in GUI mode...", "INFO") # English Log
...
def run_headless_mode(kernel_instance, preset_to_run):
    kernel_instance.write_to_log(f"Starting application in HEADLESS mode for preset: '{preset_to_run}'", "INFO") # English Log
    api_service = kernel_instance.get_service("api_server_service")
    if api_service:
        api_service.trigger_workflow_by_api(preset_to_run)
    else:
        kernel_instance.write_to_log("API service not found for headless execution.", "ERROR") # English Log
    kernel_instance.write_to_log("Headless execution finished. Application will be closed.", "INFO") # English Log
...
if __name__ == "__main__":
    ...
    # [MODIFIKASI] Aktifkan custom importer kita
    sys.meta_path.insert(0, FloworkFinder(project_root_path))

    # [MODIFIKASI] Pindahkan import PermissionHook ke sini, setelah importer aktif
    from flowork_kernel.core.permission_hook import PermissionHook
    sys.meta_path.insert(1, PermissionHook()) # Kita taruh di posisi kedua, setelah finder kita
    ...
    try:
        ...
        if args.start_server:
            ...
            if loc:
                ...
                kernel.write_to_log("API server has been force-enabled via command line argument.", "WARN") # English Log
    ...
    if args.run_preset:
        ...
        kernel.write_to_log("Flowork is now running in server-only mode. Press Ctrl+C to stop.", "SUCCESS") # English Log
        try:
            while True:
                time.sleep(1)
        except KeyboardInterrupt:
            kernel.write_to_log("Server-only mode stopped by user.", "INFO") # English Log
