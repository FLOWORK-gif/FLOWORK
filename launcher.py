...
# (DIKOMENTARI) Variabel ini tidak lagi digunakan karena library sekarang modular per-komponen.
# LIBS_ZIP_NAME = "libs.zip"
# LIBS_DIR_NAME = "libs"
MAIN_SCRIPT_BASE_NAME = "main"
# (ADDED) The PID file left by the pre_launcher
...
# (ADDED) New function to clean up the preloader popup
def cleanup_preloader():
    """
    Finds and terminates the preloader popup process after the main app has launched.
    """
    pid_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), PRELOADER_PID_FILE)
    if os.path.exists(pid_file_path):
        try:
            # Give the main UI a moment to appear before the popup disappears
            time.sleep(2)
            with open(pid_file_path, "r") as f:
                pid = int(f.read().strip())

            print(f"[LAUNCHER] Preloader found with PID: {pid}. Terminating it now.")
            # Terminate the preloader process
            if platform.system() == "Windows":
            ...
            print(f"[LAUNCHER-WARN] Could not terminate preloader process with PID {pid}: {e}")
        finally:
            # Always try to remove the pid file
            try:
                os.remove(pid_file_path)
            except OSError as e:
                print(f"[LAUNCHER-WARN] Could not remove preloader PID file: {e}")
...
def main():
    ...
    try:
        ...
        # (DIKOMENTARI) Pengecekan dan ekstraksi libs.zip tidak lagi diperlukan.
        ...
        # (DIKOMENTARI) Logika cleanup preloader ada di pre_launcher.py yang baru, jadi tidak perlu di sini.
        # cleanup_preloader()
        pass
