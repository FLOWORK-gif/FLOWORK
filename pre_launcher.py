...
# (Full code for the STABLE version with dev mode detection)
# [FIXED] Dev mode now correctly launches main.py silently and waits for a signal.
#######################################################################
...
# [ADDED] The public key required to validate devmode.on, centralized here.
...
def run_background_processes(window_instance):
    ...
    try:
        ...
        # [MODIFIED] Clean up any old signal file before starting
        if os.path.exists(MAIN_APP_READY_SIGNAL):
            os.remove(MAIN_APP_READY_SIGNAL)

        if is_dev_mode_active:
            # [MODIFIED] In dev mode, run main.py silently with poetry
            ...
            # [COMMENT] This is the original production/portable logic, it remains the same.
        ...
        # [MODIFIED] New, more reliable waiting logic.
