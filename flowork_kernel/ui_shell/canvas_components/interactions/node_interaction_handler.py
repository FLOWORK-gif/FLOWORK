...
class NodeInteractionHandler:
    ...
    def show_node_context_menu(self, event):
        ...
        component_id_to_find = node_data.get("module_id")
        manifest = None
        if module_manager:
            manifest = module_manager.get_manifest(component_id_to_find)
        if not manifest and plugin_manager:
            manifest = plugin_manager.get_manifest(component_id_to_find)
        if not manifest:
            manifest = {} # Fallback to empty dict if not found in either
