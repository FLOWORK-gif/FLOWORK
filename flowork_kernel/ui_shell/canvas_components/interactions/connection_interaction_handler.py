...
class ConnectionInteractionHandler:
    ...
    def finish_line_drawing(self, end_node_id, target_port_name=None, target_port_type='input'):
        ...
        start_node_module_id = self.canvas_manager.canvas_nodes[start_node_id]['module_id']
        start_node_manifest = None
        if module_manager:
            start_node_manifest = module_manager.get_manifest(start_node_module_id)
        if not start_node_manifest and plugin_manager:
            start_node_manifest = plugin_manager.get_manifest(start_node_module_id)
        is_source_brain = start_node_manifest and start_node_manifest.get('subtype') == 'BRAIN_PROVIDER'
    ...
    def on_line_motion(self, event):
        ...
        if is_source_brain and hovered_tags:
            ...
            if hovered_node_id and self.canvas_manager.canvas_nodes[hovered_node_id].get('module_id') == 'agent_host':
