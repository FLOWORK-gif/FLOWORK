    ...
    def on_drag_release(self, event, item_id, tree_widget):
        if item_id:
            ...
            if canvas_x0 <= x_root <= canvas_x1 and canvas_y0 <= y_root <= canvas_y1:
                ...
                if module_manager:
                    manifest = module_manager.get_manifest(component_id)
                if not manifest:
                    plugin_manager = self.kernel.get_service("plugin_manager_service")
                    if plugin_manager:
                        manifest = plugin_manager.get_manifest(component_id)
                if manifest:
                    self.canvas_manager.node_manager.create_node_on_canvas(
                        name=manifest.get('name', 'Unknown'),
                        x=world_x,
                        y=world_y,
                        module_id=component_id # Gunakan ID asli dari item yang di-drag
                    )
                else:
                    self.kernel.write_to_log(f"Drag drop failed: Component ID '{component_id}' not found in any manager.", "ERROR")
