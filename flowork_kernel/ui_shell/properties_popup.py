...
class PropertiesPopup(ttk.Toplevel):
    ...
    def _get_incoming_variables(self):
        ...
        if not module_manager and not plugin_manager: return {}
        for conn in self.canvas_manager.canvas_connections.values():
            if conn['to'] == self.node_id:
                ...
                if from_node_data:
                    ...
                    from_module_instance = self.kernel.get_component_instance(from_module_id)
                    ...
                    from_node_manifest = None
                    if module_manager and from_module_id in module_manager.loaded_modules:
                        from_node_manifest = module_manager.get_manifest(from_module_id)
                    elif plugin_manager and from_module_id in plugin_manager.loaded_plugins:
                        from_node_manifest = plugin_manager.get_manifest(from_module_id)
