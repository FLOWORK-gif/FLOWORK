...
import threading
# (ADDED) Import necessary libraries for verification
import base64
try:
...
class UpdateService(BaseService):
    ...
    """
    # (ADDED) The Public Key for verification is now self-contained within this service.
    # This makes the service 100% plug-and-play without hidden dependencies.
...
-----END PUBLIC KEY-----"""
    ...
    def __init__(self, kernel, service_id: str):
        ...
        # (MODIFIED) Changed repo_name to FLOWORK to match the logic. "ASSET" seems to be for addons.
        self.repo_name = "FLOWORK"
        self.branch = "main"
        self.fingerprint_filename = "file_fingerprints.json"
        # (ADDED) Signature file name constant
        self.signature_filename = "file_fingerprints.sig"
        self.dev_mode_file = "devmode.on"
        # (ADDED) Load public key during initialization
        self.public_key = self._load_public_key()

    # (ADDED) Function to load the public key, making the service self-reliant.
    ...
    # (MODIFIED) This function now fetches both the json and sig files, then verifies them.
    ...
    # (MODIFIED) Main function updated to incorporate the new logic
    def run_update_check(self):
        """
        The main method called by StartupService to perform the update.
        """
        # (ADDED) Check for devmode.on file
        ...
        # (MODIFIED) Call the new verification function
        ...
        if success_count > 0:
            ...
            if event_bus:
                ...
                # (MODIFIED) Give a brief moment for file I/O to complete before requesting restart
