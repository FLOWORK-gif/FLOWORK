...
# [FIXED V3] Final fix for parallel execution. An error in one branch will no longer stop the entire workflow
# as long as at least one other parallel branch succeeds.
#######################################################################
...
class WorkflowExecutorService(BaseService):
    ...
    # ... (code dari awal sampai _find_and_execute_next_nodes tidak berubah, jadi aku singkat) ...
    ...
    def execute_workflow_synchronous(self, nodes, connections, initial_payload, logger, status_updater, highlighter, ui_callback, workflow_context_id, mode, job_status_updater, on_complete=None, start_node_id=None):
        ...
        try:
            ...
            if hasattr(self.kernel.root, 'display_permission_denied_popup'):
    ...
    def _find_and_execute_next_nodes(self, current_node_id, execution_result, nodes, connections, log, update_status, highlight, run_on_ui, workflow_context_id="default_workflow", mode: str = 'EXECUTE'):
        ...
        for conn_id, conn_data in connections.items():
            if conn_data.get('from') == current_node_id and conn_data.get('type', 'data') != 'tool':
                ...
                if expected_output_name is not None:
                    # Case 1: The module specified an output port name (e.g., 'true' or 'false')
                    ...
                    # Case 2: The module did NOT specify an output port (it's a dynamic port or a simple module)
                    # We fire ALL connected data ports.
                    self._record_connection_event(workflow_context_id, conn_id, payload_for_next)
                    next_nodes_to_execute.append((conn_id, conn_data.get('to'), payload_for_next))
        ...
        # [FIX] This is the new, robust parallel execution logic
        if len(next_nodes_to_execute) > 1:
            ...
            # [MODIFIED] New logic: If there are ANY successful results, we consider the overall step a success.
            if successful_results:
                # We return the result of the last successful branch to finish.
                # A more complex strategy could involve merging payloads, but this is safer.
                return successful_results[-1]
            elif exceptions:
                # Only if ALL branches failed do we propagate an error.
                log(f"All {len(exceptions)} parallel branches failed. Propagating the first error.", "ERROR")
                return exceptions[0]
            else:
                # This case (no successes, no exceptions) should be rare, but we handle it.
                return execution_result
