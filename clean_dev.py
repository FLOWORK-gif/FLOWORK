...
# (MODIFIED: Now also targets 'vendor' folders and '.vendor_hash' files)
#######################################################################
...
def clean_project_artifacts():
    ...
    TARGETS: build_fingerprint.json, *.aola_flowork, vendor/, .vendor_hash, *.py.tmp.build, *.pyi
    """
    ...
    # [MODIFIKASI] Menambahkan '.vendor_hash' dan 'build_fingerprint.json' ke daftar target file
    files_to_delete_exact = ['.vendor_hash', 'TES_build_fingerprint.json']
    files_to_delete_endswith = ['.pyi','.pyd','.original']
    # [MODIFIKASI] Mengubah 'TES_vendor' menjadi 'vendor' agar sesuai target
    folders_to_delete = ['vendor', 'TES_vendor']
