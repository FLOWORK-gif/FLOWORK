...
class CoreUIProvider(BaseModule, BaseUIProvider):
    ...
    def __init__(self, module_id, services):
        ...
        if hasattr(self, 'logger') and callable(self.logger):
            self.logger("<<<<< [MATA-MATA] CoreUIProvider __init__ BERHASIL DIJALANKAN! >>>>>", "SUCCESS")
    def execute(self, payload, config, status_updater, ui_callback, mode='EXECUTE'):
        status_updater("No action", "INFO")
        return payload
    def get_ui_tabs(self):
        self.logger("[MATA-MATA] CoreUIProvider: get_ui_tabs() dipanggil oleh Kernel.", "DEBUG")
        all_pages = [
            {
                "key": "settings",
    ...
    def get_menu_items(self):
        self.logger("[MATA-MATA] CoreUIProvider: get_menu_items() dipanggil oleh Kernel.", "DEBUG") # English Log
