...
# (FIXED) Changed relative imports to absolute imports to prevent ImportError on dynamic loading.
from plugins.system_diagnostics_plugin.scanners.base_scanner import BaseScanner
from plugins.system_diagnostics_plugin.diagnostics_page import DiagnosticsPage
        ...
        if os.path.isdir(scanners_dir):
            for entry in os.scandir(scanners_dir):
                if entry.name.endswith('.py') and not entry.name.startswith('__'):
                    # (FIXED) Relative import fix: Construct the full module path
                    ...
                    try:
                        ...
                        # (COMMENT) Added more detailed logging for import errors
                        self.logger(f"Diagnostics: Failed to import scanner from '{entry.name}': {e}", "ERROR")
                        pass
        return all_scanners
