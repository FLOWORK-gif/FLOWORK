...
class MarketplacePage(ttk.Frame):
    ...
    def _fetch_all_data_worker(self):
        ...
        def fetch_component_data(comp_type):
            # MODIFIED: Use specific API calls for special component types.
            if comp_type == 'presets':
                success_local, local_data = self.api_client.get_presets()
            elif comp_type == 'ai_models':
                success_local, local_data = self.api_client.get_ai_models()
            else:
