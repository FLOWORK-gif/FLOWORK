...
class ImageViewerModule(BaseModule, IExecutable, IConfigurableUI, IDataPreviewer):
    ...
    def __init__(self, module_id, services):
        ...
        # (COMMENT) Changed from a single variable to a dictionary to handle multiple popups from different nodes of this type.
    ...
    def _show_image_popup(self, node_instance_id, image_path):
        ...
        # (COMMENT) Check if a popup for this specific node_id already exists.
        ...
        # (COMMENT) Store the new popup in the dictionary using its unique node_id as the key.
        self._active_popups[node_instance_id] = popup

        def _on_close():
            self.logger("Image viewer was closed manually by the user.", "DEBUG") # English Log
            # (COMMENT) Remove the specific node's entry from the dictionary.
        ...
        popup.transient()
        popup.grab_set()
        # popup.wait_window() # (COMMENT) This call is blocking and causes race conditions in parallel execution.

    def execute(self, payload: dict, config: dict, status_updater, ui_callback, mode='EXECUTE'):
        # (ADDED) Get the unique ID of the node instance that is currently running.
        ...
        # (COMMENT) Pass the unique node_id to the UI function so it knows which popup belongs to which node.
