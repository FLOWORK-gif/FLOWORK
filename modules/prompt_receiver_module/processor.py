...
class PromptReceiverModule(BaseModule, IExecutable, IConfigurableUI, IDataPreviewer):
    ...
    def _handle_prompt_event(self, event_data):
        ...
        file_path = event_data.get("file_path") # [PENAMBAHAN] Ambil file_path dari event
        self.logger(f"Received prompt for node '{self.node_instance_id}': '{prompt[:50]}...'", "INFO")
        if file_path:
            self.logger(f"Received file path for node '{self.node_instance_id}': '{file_path}'", "INFO") # ENGLISH LOG
        new_payload = {
            "data": {
                "prompt": prompt,
                "file_path": file_path # [PENAMBAHAN] Masukkan file_path ke payload baru
            },
            "history": []
        }
        if self.kernel:
